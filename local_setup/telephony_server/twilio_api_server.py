...
def populate_ngrok_tunnels():
    ...
    telephony_url, eburon_url = None, None
    ...
    if response.status_code == 200:
        ...
        for tunnel in data['tunnels']:
            ...
            elif tunnel['name'] == 'eburon-app':
                eburon_url = tunnel['public_url'].replace('https:', 'wss:')

        return telephony_url, eburon_url
    else:
...
async def make_call(request: Request):
    try:
        ...
        telephony_host, eburon_host = populate_ngrok_tunnels()

        print(f'telephony_host: {telephony_host}')
        print(f'eburon_host: {eburon_host}')

        try:
            call = twilio_client.calls.create(
                to=call_details.get('recipient_phone_number'),
                from_=twilio_phone_number,
                url=f"{telephony_host}/twilio_connect?eburon_host={eburon_host}&agent_id={agent_id}",
                method="POST",
                record=True
            )
...
async def twilio_connect(eburon_host: str = Query(...), agent_id: str = Query(...)):
    try:
        response = VoiceResponse()

        connect = Connect()
        eburon_websocket_url = f'{eburon_host}/chat/v1/{agent_id}'
        connect.stream(url=eburon_websocket_url)
        print(f"websocket connection done to {eburon_websocket_url}")
