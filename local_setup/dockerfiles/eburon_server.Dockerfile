...
# Install common dependencies that eburon requires
RUN --mount=type=cache,target=/root/.cache/pip \
    pip install \
    python-dotenv \
    pydantic \
...
# Install eburon package with verbose output for debugging
RUN --mount=type=cache,target=/root/.cache/pip \
    pip install --verbose git+https://github.com/bolna-ai/bolna@master || \
    (echo "Failed to install eburon package. See error above." && exit 1)
