...
class EburonEdgeConfig(BaseModel):
    voice: str = "ep_001"
    model: str = "nike"
    base_url: Optional[str] = "http://localhost:17493"
    sampling_rate: Optional[int] = 24000
...
class Synthesizer(BaseModel):
    ...
    def preprocess(cls, values):
        ...
        if provider == "elevenlabs":
            ...
            if isinstance(config, dict):
                values["provider_config"] = KokoroConfig(**config)
        elif provider == "ollama_tts":
            if isinstance(config, dict):
                values["provider_config"] = OllamaTTSConfig(**config)
        ...
        elif provider == "eburon_edge":
            if isinstance(config, dict):
                values["provider_config"] = EburonEdgeConfig(**config)
