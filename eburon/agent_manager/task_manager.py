...
from eburon.helpers.function_calling_helpers import trigger_api, computed_api_response
from eburon.helpers.conversation_history import ConversationHistory
from .base_manager import BaseManager
from .interruption_manager import InterruptionManager
from eburon.agent_types import *
from eburon.providers import *
from eburon.enums import TelephonyProvider, LogComponent, LogDirection, HangupReason
from eburon.exceptions import EburonComponentError, LLMError, SynthesizerError, TranscriberError
from eburon.prompts import *
from eburon.helpers.language_detector import LanguageDetector
from eburon.transcriber.transcriber_pool import TranscriberPool
from eburon.synthesizer.synthesizer_pool import SynthesizerPool
from eburon.helpers.utils import (
...
class TaskManager(BaseManager):
        ...
        **kwargs,
        ...
        if task_id == 0:
            ...
            if self.conversation_config is not None:
                transcriber = self.task_config["tools_config"].get("transcriber")
                if transcriber is not None:
                    self.minimum_wait_duration = transcriber.get("endpointing", 400)
                else:
                    self.minimum_wait_duration = 400
    ...
    def __setup_synthesizer(self, llm_config=None):
        if self._is_conversation_task():
            transcriber = self.task_config["tools_config"].get("transcriber")
            if transcriber is not None:
                self.kwargs["use_turbo"] = transcriber.get("language") == DEFAULT_LANGUAGE_CODE
            else:
                self.kwargs["use_turbo"] = True
    ...
    async def _run_llm_task(self, message):
        ...
        try:
            ...
            self.llm_task = None
        except EburonComponentError as e:
    ...
    async def _end_call_on_component_error(self, error, hangup_detail):
        ...
        if self.run_id and not self._error_logged:
            if isinstance(error, EburonComponentError):
        ...
        try:
            if self._is_conversation_task():
                ...
                try:
                    ...
                    if self.run_id and not self._error_logged:
                        if isinstance(e, EburonComponentError):
                ...
                for attr, cls, provider in [
                    ...
                    if task and task.done() and not task.cancelled():
                        exc = task.exception()
                        if exc is not None:
                            raise exc if isinstance(exc, EburonComponentError) else cls(str(exc), provider=provider)
