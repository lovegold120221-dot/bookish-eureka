...
from eburon.helpers.utils import now_ms, compute_function_pre_call_message, convert_to_request_log
...
class GeminiLLM(BaseLLM):
    ...
    def __init__(self, max_tokens=100, buffer_size=40, model="gemini-2.5-flash", temperature=0.1, **kwargs):
        ...
        eburon_tools = kwargs.get("api_tools", {}).get("tools", [])

        gemini_declarations = []
        if eburon_tools:
            if isinstance(eburon_tools, str):
                try:
                    eburon_tools = json.loads(eburon_tools)
                except json.JSONDecodeError:
                    logger.error("Failed to parse tool definitions as JSON")
                    eburon_tools = []

            for tool in eburon_tools:
        ...
        # Keep raw eburon tools list for required-param validation at call time
        self.eburon_tools_raw = eburon_tools if isinstance(eburon_tools, list) else []
        self.thinking_budget = kwargs.get("thinking_budget", 0)
        self.run_id = kwargs.get("run_id", None)
        self.language = kwargs.get("language", "en")

    def _prepare_history(self, messages):
        """Translate Eburon roles (OpenAI-style) to Gemini-style roles and parts."""
        system_instruction = ""
        history = []
                    ...
                    for part in chunk.candidates[0].content.parts:
                        if part.function_call:
                            ...
                            tool_spec = next(
                                (
                                    t
                                    for t in self.eburon_tools_raw
