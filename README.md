...
This is an end-to-end open source production ready framework for quickly building LLM-based voice-driven conversational applications.
...
## Components
This platform helps you create AI Voice Agents which can be instructed to do tasks:
...
```mermaid
graph LR;
    A[Open source orchestration] -->B[Hosted APIs];
...
2. Voice AI server: for creating and handling agents 
...
```bash
docker compose up -d voice-app twilio-app
# or
docker compose up -d voice-app plivo-app
```
...
import asyncio
from voice_agent.assistant import Assistant
from voice_agent.models import LlmAgent, SimpleLlmAgent
...
This platform uses LiteLLM package to support multiple LLM integrations.
...
<a href="https://calendly.com/voice-ai/30min"><img alt="Schedule a meeting" src="https://cdn.cookielaw.org/logos/122ecfc3-4694-42f1-863f-2db42d1b1e68/0bcbbcf4-9b83-4684-ba59-bc913c0d5905/c21bea90-f4f1-43d1-8118-8938bbb27a9d/logo.png" /></a>
...
2. Add the telephony-specific input handler file in [input_handlers/telephony_providers](https://github.com/voice-ai/platform/tree/master/input_handlers/telephony_providers) writing custom functions extending from the [telephony.py](https://github.com/voice-ai/platform/blob/master/input_handlers/telephony.py) class
   1. This file will mainly contain how different types of event packets are being ingested from the telephony provider
3. Add telephony-specific output handler file in [output_handlers/telephony_providers](https://github.com/voice-ai/platform/tree/master/output_handlers/telephony_providers) writing custom functions extending from the [telephony.py](https://github.com/voice-ai/platform/blob/master/output_handlers/telephony.py) class
...
4. Lastly, you'll have to write a dedicated server like the example [twilio_api_server.py](https://github.com/voice-ai/platform/blob/master/local_setup/telephony_server/twilio_api_server.py) provided in [local_setup](https://github.com/voice-ai/platform/blob/master/local_setup/telephony_server) to initiate calls over websockets.


## Contributing
We love all types of contributions: whether big or small helping in improving this community resource.

1. There are a number of [open issues present](https://github.com/voice-ai/platform/issues) which can be good ones to start with
